...
This is the workhorse "sigmoid" function, usually written $$\sigma(a)$$.  (*And doesn't it remind you of the Elo rating probability?*)  
...
where $$\alpha_k$$ controls how fast we update our estimate of$$\mathbf{w}$$.  Crucially, note it may be constant or adaptive (i.e. depend on $$k$$).
...
$$
\begin{equation}
\mathbf{w}_{k+1} = \mathbf{w}_k - \alpha_k \ \big(\sigma(\mathbf{w}_k^T \mathbf{x}_i) - t_i \big) \mathbf{x}_i
\label{eq:SGD}
\end{equation}
$$
...
since $$\mathbf{w}^T \mathbf{x}_{(ij)}_ = w_i - w_j$$ or in Elo notation, $$\text{elo}_i - \text{elo}_j$$.  Then given some initial estimate of the weight vector $$\mathbf{w}_0$$, we can hone our estimate of the weights by iteratively applying an SGD step on the data,

$$
\mathbf{w}_{k+1} = \mathbf{w}_k - \alpha \ (\sigma (\mathbf{w}_k^T \mathbf{x}_{(ij)}) - 1) \mathbf{x}
$$
...
So, from a high level, Elo ratings are actually weights of a logistic regression to predict pairwise game outcomes, which we learn through SGD-like updates over streaming data.
...
- A common improvement to first order methods like GD/SGD is incorporation of **momentum**.  A simple example is the following modified gradient step:
