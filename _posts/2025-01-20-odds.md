...
Go check out the [Wikipedia page on odds](https://en.wikipedia.org/wiki/Odds), the whole intro is mostly doing the map between a phrase like "X to Y against" and a probability (i.e. a number between 0 and 1).  But in each case, it's a direct mapping: "3 to 2 against" maps to "40\%" probability (aka 0.4), etc.

Odds and probability are *different* in that, very often, odds are connected to a **bet** and therefore some form of profit/loss.  

Here's how we can connect those ideas.  Define $$w$$ as the amount you win for a \$1 bet (that is, the amount you would win in addition to getting your dollar back), and let $$p$ be the probability of winning.  Then the *expected value* (EV) of your bet is
...
For example, if an event's odds are "3 to 1" (aka 3), this implies probability $$p=0.75$$, and so a bookie could offer the fair bet of \$1 stake gets winnings of $$w = 0.25 / 0.75 = 1 / 3 = 0.33$.  Since the event has good odds of happening, you're only offered 33 cents on a 1 dollar bet.
...
Decimal odds represent the winnings *and* the stake.  Specifically, for decimal odds $$D$$, a \$$1 bet wins $$D-1$$ dollars.  For example, a bet of one dollar on decimal odds of 2 earns you \$$2: your original dollar back, and one more.
...
Weirdest ones.  The first idea is to rebase everything to be off \$$100 instead of \$$1, and then to keep that spirit up by having all the odds stay triple digits, regardless how low they go, by defining the odds differently low and high.

- Positive moneyline odds represent winnings on a \$$100 wager.  For example, 3:1 (against) odds would have a moneyline of +300, or winnings of \$$300 on a \$$100 bet.

- Negative moneyline odds represent how much money must be wagered to win \$$100.  For example, 1:3 (against, aka 3:1 in favor) odds would have a moneyline of -300, or winnings of \$$100 on a \$$300 stake.
...
Let's continue with this case of two teams playing each other.  Again, say the moneyline for both teams is -110.  We know this implies $$p=110/(110+100)=0.524$$, but that's for both teams which adds up to $$0.524\ast 2=1.0476$$!  The chance of one of the teams winning should be 100\%, not 104\%.  This reveals the vig of 0.0476, which on \$210 of winnings is $$210\ast 0.0476=10$$.  It also reveals the bookmaker's true estimate of the probability for both teams as an even 50\%.
