---
layout: post
title: "2 - Peeking under the hood with a linear model (AI series)"
categories: journal
date: 2020-04-02
...
# Part 2: Peeking under the hood with a linear model
...
$$
\text{MSE} = \frac{1}{6}\Big((150 - 160)^2 + (175 - 220)^2 + (800 - 320)^2 + (900 - 270)^2 + (950 - 420)^2 + (1200 - 300)^2\Big)
$$
...
<img align="center" width="100%" src="{{ site.github.url }}/images/ai/data_guesses.png" alt="Fitting a linear model">
...
Now comes the real test:  we spent $$x=\$120$$ on advertising this year, and using our model, we predict we will make $$y=\$980.50$$. 

<img align="center" width="100%" src="{{ site.github.url }}/images/ai/data_line.png" alt="Final fitted linear model">

Unfortunately, when summer's over, we find we only made \$850.  Not too far from our model's prediction, but still kind of a bummer.  Hopefully we kept the limitations of our model in our mind before making any dangerous financial commitments.
