...
which is just fine.  We are getting into the realm of autoregressive models here (Elo is an AR(1) model, thus the [connection to SGD](https://stmorse.github.io/journal/Elo.html)), but I think we can avoid any of that theory for this so long as we're bought into the "expected new value should equal old actual value".  
...
$$
\begin{equation}
\frac{A(x; i \text{ win})}{A(x; i \text{ lose})} = \frac{\mathbb{E}[M(z)|i \text{ lose}]}{\mathbb{E}[M(z)|i \text{ wins}]}
\label{eq:goal}
\end{equation}
$$
...
I once taught with an ordained Catholic priest, who also held a PhD in differential equations.  Among many memorable things this amazing man did, I recall that he often ended his in-class derivations with not QED, but "W.L." ... for "Wah lah!"
...
<img align="center" width="100%" src="{{ site.github.url }}/images/pdiffs_elodiffs_jointplot.png" alt="Point diff vs Elo diff">
